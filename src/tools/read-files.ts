...
export interface ReadFilesToolOptions {
  cwd?: string;
...
interface ReadFilesInput extends ToolInput {
  paths?: unknown;
  encoding?: unknown;
  maxBytes?: unknown;
...
export class ReadFilesTool implements Tool {
  ...
  constructor(options: ReadFilesToolOptions = {}) {
  ...
  async run(input: ReadFilesInput): Promise<ToolExecutionResult> {
    const normalizedPaths = normalizePaths(input.paths);
    if (!normalizedPaths) {
      return this.createErrorResult('Read-files tool requires a non-empty "paths" array of strings.', {
        paths: input.paths,
      });
    }
